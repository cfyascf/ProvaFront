...
function App() {
  ...
  const getUsers = async () => {
    try {
      const res = await axios.get("http://100.20.92.101:8080/api/person");
      setList(res.data);
  ...
  const registerUser = async () => {
    try { 
      await axios.post("http://100.20.92.101:8080/api/person",{
  ...
  const deleteUser = async (id) => {
    try { 
      await axios.delete(`http://100.20.92.101:8080/api/person/${id}`);
      getUsers()
    } catch (error) {
      console.error("Erro ao inserir", error);
    }
